#!/usr/bin/env python3
"""Deribit Options Skew Bot.

Trades based on Deribit options market sentiment:
  - 25-delta put/call skew for directional bias
  - DVOL (implied vol index) as a regime gate

Fetches live Deribit skew each cycle. For the DVOL gate, fetches recent
hourly DVOL history and computes a z-score.

This is a LIVE signal strategy — skew is not available historically.
Paper trading is fully supported to validate the signal.

Usage:
    uv run python scripts/bots/deribit_skew_bot.py --paper --asset btc --timeframe 15m
    uv run python scripts/bots/deribit_skew_bot.py --paper --asset btc --timeframe 60m
"""

import argparse
import os
import signal
import time
from datetime import datetime

from polymarket_algo.core.config import LOCAL_TZ, TIMEZONE_NAME, Config
from polymarket_algo.data.binance import fetch_klines
from polymarket_algo.data.deribit import (
    compute_deribit_candles,
    fetch_dvol_hourly,
    fetch_live_skew,
)
from polymarket_algo.executor.client import PolymarketClient
from polymarket_algo.executor.trader import PaperTrader, TradingState
from polymarket_algo.strategies.deribit_skew import DeribitSkewStrategy

TF_SECONDS: dict[str, int] = {"15m": 900, "60m": 3600}
ASSET_TO_SYMBOL: dict[str, str] = {"btc": "BTCUSDT", "eth": "ETHUSDT"}
DERIBIT_CURRENCY: dict[str, str] = {"btc": "BTC", "eth": "ETH"}
CANDLE_BUFFER = 40  # bars for IV z-score + buffer
DVOL_LOOKBACK_MS = 7 * 24 * 3600 * 1000  # 7 days for z-score window

running = True


def handle_signal(sig, _frame):
    global running
    print("\n[bot] Shutting down gracefully...")
    running = False


def log(msg: str):
    ts = datetime.now(LOCAL_TZ).strftime("%H:%M:%S")
    print(f"[{ts}] {msg}", flush=True)


def main():
    global running
    signal.signal(signal.SIGINT, handle_signal)
    signal.signal(signal.SIGTERM, handle_signal)

    parser = argparse.ArgumentParser(description="Deribit Options Skew Bot")
    parser.add_argument("--paper", action="store_true")
    parser.add_argument("--live", action="store_true")
    parser.add_argument("--asset", choices=["btc", "eth"], default=os.getenv("ASSET", "btc").lower())
    parser.add_argument("--timeframe", choices=["15m", "60m"], default=os.getenv("TIMEFRAME", "15m"))
    parser.add_argument("--put-skew-thresh", type=float, default=-5.0, metavar="F")
    parser.add_argument("--call-skew-thresh", type=float, default=2.0, metavar="F")
    parser.add_argument("--iv-gate-z", type=float, default=2.0, metavar="F")
    parser.add_argument("--size", type=float, default=float(os.getenv("BET_AMOUNT", str(Config.BET_AMOUNT))))
    parser.add_argument("--bankroll", type=float)
    args = parser.parse_args()

    paper_mode = not args.live if args.live else (True if args.paper else Config.PAPER_TRADE)
    timeframe = args.timeframe
    poly_tf = "1h" if timeframe == "60m" else timeframe
    window_seconds = TF_SECONDS[timeframe]
    asset = args.asset
    symbol = ASSET_TO_SYMBOL.get(asset, "BTCUSDT")
    deribit_currency = DERIBIT_CURRENCY.get(asset, "BTC")

    strategy = DeribitSkewStrategy()

    client = PolymarketClient(asset=asset)
    state = TradingState.load()
    if args.bankroll:
        state.bankroll = args.bankroll

    trader = PaperTrader() if paper_mode else __import__("polymarket_algo.executor.trader", fromlist=["LiveTrader"]).LiveTrader()
    log(f"Strategy: {strategy.name} | {asset.upper()}/{timeframe} | {'PAPER' if paper_mode else 'LIVE'}")
    log(f"Params: put_thresh={args.put_skew_thresh}, call_thresh={args.call_skew_thresh}, "
        f"iv_gate_z={args.iv_gate_z}, size=${args.size:.2f}")
    log(f"Timezone: {TIMEZONE_NAME} | Bankroll: ${state.bankroll:.2f}")
    log("")

    bet_timestamps: set[int] = {t.timestamp for t in state.trades}
    pending: list = []

    while running:
        try:
            now = int(time.time())
            target_ts = ((now // window_seconds) + 1) * window_seconds
            seconds_until_target = target_ts - now

            for trade in list(pending):
                market = client.get_market(trade.timestamp)
                if market and market.closed and market.outcome:
                    state.settle_trade(trade, market.outcome, market)
                    emoji = "+" if trade.pnl > 0 else "-"
                    log(f"[{emoji}] Settled: {trade.direction.upper()} -> {market.outcome.upper()} "
                        f"| PnL: ${trade.pnl:+.2f} | Bankroll: ${state.bankroll:.2f}")
                    pending.remove(trade)
                    state.save()

            can_trade, reason = state.can_trade()
            if not can_trade:
                time.sleep(10)
                continue

            if target_ts in bet_timestamps:
                time.sleep(5)
                continue

            if seconds_until_target > Config.ENTRY_SECONDS_BEFORE:
                time.sleep(1)
                continue

            now_ms = int(time.time() * 1000)
            candle_ms = CANDLE_BUFFER * window_seconds * 1000
            binance_tf = "1h" if timeframe == "60m" else timeframe

            try:
                candles = fetch_klines(symbol, binance_tf, now_ms - candle_ms, now_ms)
                iv_df = fetch_dvol_hourly(deribit_currency, now_ms - DVOL_LOOKBACK_MS, now_ms)
            except Exception as e:
                log(f"Data fetch error: {e}")
                time.sleep(10)
                continue

            if candles.empty:
                log("No candles")
                bet_timestamps.add(target_ts)
                time.sleep(5)
                continue

            candles = candles.set_index("open_time").sort_index()
            enriched = compute_deribit_candles(iv_df, candles)

            # Fetch live skew from Deribit
            log("Fetching live Deribit skew...")
            live_skew = fetch_live_skew(deribit_currency)
            log(f"Live skew: {live_skew:.2f}%" if live_skew is not None else "Live skew: unavailable")

            eval_params = {
                "put_skew_thresh": args.put_skew_thresh,
                "call_skew_thresh": args.call_skew_thresh,
                "iv_gate_z": args.iv_gate_z,
                "live_skew": live_skew,
                "size": args.size,
            }

            result = strategy.evaluate(enriched, **eval_params)

            last = result.iloc[-1]
            raw_signal = int(last["signal"])
            bet_size = float(last["size"])

            if raw_signal == 0 or bet_size <= 0:
                iv_z = enriched["iv_zscore"].iloc[-1] if "iv_zscore" in enriched.columns else 0.0
                log(f"No signal (iv_z={iv_z:.2f}, skew={live_skew}) — skip")
                bet_timestamps.add(target_ts)
                time.sleep(5)
                continue

            direction = "up" if raw_signal == 1 else "down"
            iv_z = enriched["iv_zscore"].iloc[-1] if "iv_zscore" in enriched.columns else 0.0
            log(f"Signal: {direction.upper()} | skew={live_skew:.2f}%, iv_z={iv_z:.2f} | size=${bet_size:.2f}")

            can_trade, reason = state.can_trade(bet_size=bet_size)
            if not can_trade:
                log(f"Skipping: {reason}")
                bet_timestamps.add(target_ts)
                time.sleep(5)
                continue

            market = client.get_market(target_ts)
            if not market or not market.accepting_orders:
                log(f"Market unavailable for ts={target_ts}")
                bet_timestamps.add(target_ts)
                time.sleep(5)
                continue

            trade = trader.place_bet(
                market=market,
                direction=direction,
                amount=bet_size,
                confidence=0.55,
                streak_length=0,
                gate_name="deribit_skew",
            )

            if trade is None:
                log("Order rejected")
                bet_timestamps.add(target_ts)
                continue

            state.record_trade(trade)
            bet_timestamps.add(target_ts)
            pending.append(trade)
            state.save()
            log(f"Placed: {direction.upper()} ${bet_size:.2f} on {market.slug} "
                f"| Daily: {state.daily_bets} bets, PnL: ${state.daily_pnl:+.2f}")

            time.sleep(5)

        except KeyboardInterrupt:
            break
        except Exception as e:
            log(f"Error: {e}")
            time.sleep(10)

    if pending:
        state.mark_pending_as_force_exit("shutdown")
    state.save()
    log(f"State saved. Bankroll: ${state.bankroll:.2f} | Session: {state.daily_bets} bets, PnL: ${state.daily_pnl:+.2f}")


if __name__ == "__main__":
    main()
