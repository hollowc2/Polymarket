#!/usr/bin/env python3
"""BTC/ETH Correlation Break Bot.

Trades when BTC and ETH correlation breaks down and one asset diverges
significantly from the other. Bets on laggard catch-up or leader reversion.

The strategy fetches both BTC and ETH candles internally each cycle.
This bot can trade either BTC or ETH markets depending on --asset.

Usage:
    uv run python scripts/bots/corr_break_bot.py --paper --asset btc --timeframe 5m
    uv run python scripts/bots/corr_break_bot.py --paper --asset eth --timeframe 15m
"""
...
from polymarket_algo.strategies.corr_break import CorrBreakStrategy

TF_SECONDS: dict[str, int] = {"5m": 300, "15m": 900}
ASSET_TO_SYMBOL: dict[str, str] = {"btc": "BTCUSDT", "eth": "ETHUSDT"}
CANDLE_BUFFER = 60  # bars — enough for corr_window + buffer
...
def main():
    ...
    parser = argparse.ArgumentParser(description="BTC/ETH Correlation Break Bot")
    ...
    parser.add_argument("--corr-window", type=int, default=20, metavar="N")
    parser.add_argument("--corr-thresh", type=float, default=0.7, metavar="F")
    parser.add_argument("--spread-z-thresh", type=float, default=1.5, metavar="F")
    ...
    secondary_symbol = "ETHUSDT" if asset == "btc" else "BTCUSDT"

    strategy = CorrBreakStrategy()
    eval_params = {
        "corr_window": args.corr_window,
        "corr_thresh": args.corr_thresh,
        "spread_z_thresh": args.spread_z_thresh,
        "primary_asset": asset,
        "size": args.size,
    }
    ...
    log(f"Params: corr_window={args.corr_window}, corr_thresh={args.corr_thresh}, "
        f"spread_z={args.spread_z_thresh}, size=${args.size:.2f}")
    ...
    while running:
        try:
            ...
            try:
                candles = fetch_klines(symbol, timeframe, now_ms - candle_ms, now_ms)
                secondary = fetch_klines(secondary_symbol, timeframe, now_ms - candle_ms, now_ms)
            except Exception as e:
                log(f"Binance fetch error: {e}")
                time.sleep(10)
                continue

            if candles.empty or secondary.empty:
            ...
            secondary = secondary.set_index("open_time").sort_index()

            result = strategy.evaluate(candles, secondary_candles=secondary, **eval_params)
            ...
            if raw_signal == 0 or bet_size <= 0:
                log("No correlation break signal — skip")
                bet_timestamps.add(target_ts)
                time.sleep(5)
                continue
